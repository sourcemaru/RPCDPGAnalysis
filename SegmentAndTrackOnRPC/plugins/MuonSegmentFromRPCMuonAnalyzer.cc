  ...
  if ( hists_.count(runNumber) == 0 ) {
    ...
    for ( const DTChamber* ch : dtGeom->chambers() ) {
      ...
      if ( !h.hXYExpBarrelByWheel_[wh] ) {
        ...
        h.hXYRecBarrelByWheel_[wh] = dir_wheel.make<TH2F>("hXYRec", ("Expected points matched to Segment "+whStr+";X (cm);Y (cm)").c_str(), 3200, -800, 800, 3200, -800, 800);
    ...
    for ( const CSCChamber* ch : cscGeom->chambers() ) {
      ...
      if ( !h.hXYExpEndcapByDisk_[di] ) {
        ...
        h.hXYRecEndcapByDisk_[di] = dir_disk.make<TH2F>("hXYRec", ("Expected points matched to Segment "+diStr+";X (cm);Y (cm)").c_str(), 3200, -800, 800, 3200, -800, 800);
