...
hWDEffProfRPCMonitor = TProfile2D("hWDEffProfRPCMonitor", "hWDEffProf_RPCMonitor;;Station/Ring", 13, 0, 13, 4, 0.5, 4.5)
hWDEffProfRPCMonitor = TProfile2D("hWDEffProfSingleMuon", "hWDEffProf_SingleMuon;;Station/Ring", 13, 0, 13, 4, 0.5, 4.5)
for i, label in enumerate(["RE%+d" % d for d in range(-4, 0)]
    ...
    hWDEffProfRPCMonitor.GetXaxis().SetBinLabel(i+1, label)
    hWDEffProfSingleMuon.GetXaxis().SetBinLabel(i+1, label)
for ch in chMap:
    ...
    if ch.startswith("W"):
        ...
        hWDEffProfRPCMonitor.Fill(wheelStr, station, eff0)
        hWDEffProfSingleMuon.Fill(wheelStr, station, eff1)
    else:
        ...
        hWDEffProfRPCMonitor.Fill(diskStr, ring, eff0)
        hWDEffProfSingleMuon.Fill(diskStr, ring, eff1)
...
hWDEffProfRPCMonitor.SetMarkerSize(1.5)
hWDEffProfRPCMonitor.SetMaximum(100)
hWDEffProfRPCMonitor.SetMinimum(90)
cWDEffProfRPCMonitor = TCanvas("cWDEffProfRPCMonitor", "cWDEffProfRPCMonitor", 1300/2, 400)
hWDEffProfRPCMonitor.Draw("COLZtext45")
cWDEffProfRPCMonitor.Print("%s.png" % cWDEffProfRPCMonitor.GetName())
cWDEffProfRPCMonitor.Print("%s.pdf" % cWDEffProfRPCMonitor.GetName())
cWDEffProfRPCMonitor.Print("%s.C" % cWDEffProfRPCMonitor.GetName())

hWDEffProfSingleMuon.SetMarkerSize(1.5)
hWDEffProfSingleMuon.SetMaximum(100)
hWDEffProfSingleMuon.SetMinimum(90)
cWDEffProfSingleMuon = TCanvas("cWDEffProfSingleMuon", "cWDEffProfSingleMuon", 1300/2, 400)
hWDEffProfSingleMuon.Draw("COLZtext45")
cWDEffProfSingleMuon.Print("%s.png" % cWDEffProfSingleMuon.GetName())
cWDEffProfSingleMuon.Print("%s.pdf" % cWDEffProfSingleMuon.GetName())
cWDEffProfSingleMuon.Print("%s.C" % cWDEffProfSingleMuon.GetName())
