...
for fName in sys.argv[1:]:
    ...
    for line in open(fName).readlines():
        ...
        if name not in rollrun: rollrun[name] = []
...
for name in l3:
    ...
    for i in range(len(effhist[name])):
        if effhist[name][i] == 0.0 : 
            ban += 1
            strange.append(rollrun[name][i])
            continue
        summation += effhist[name][i]

    nominal = summation/(len(effhist[name])-ban)
    linename = "3    " + name + "    " + str(nominal) + "    " + str(strange) + "\n"
...
for name in l4:
    ...
    for numbering in range(1, len(cat4history[name])):
        ...
        if abs(eff1-eff2) > 0.3:
            effgroups.append([])
        if eff1 < 0.01 : strange.append(rollrun[name][numbering-1])
    ...
    linename = "4    " + name + "    " + str(nominal) + "    " + str(strange) + "\n"
